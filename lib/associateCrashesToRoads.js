...
function searchRoads(crash, roads) {
    var lat = crash.location.latitude;
    var lon = crash.location.longitude;
...
function associateCrashesToRoads(crashDB, roadFile, crashTable) {
    ...
    bikeSafetyDB.authWithCustomToken(process.env.FIREBASE_SECRET, function(error, authData) {
        ...
        var crashDB = bikeSafetyDB.child(crashTable);
...
//associateCrashesToRoads('https://bikesafety.firebaseio.com', 'html/src/data/durham-bike-lanes.geojson', 'Crashes_Sanitzed');
